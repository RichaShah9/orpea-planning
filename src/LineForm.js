...
function LineForm({handleClose, open, fromDate, date}) {
    ...
    const [contract, setContract] = React.useState({employmentContractId: '', serviceId: ''});
    ...
    const handleSave = React.useCallback(() => {
        ...
        const object = {
            action: 'com.axelor.apps.orpea.planning.web.EmploymentContractController:generateEmployeeMonth',
            data: {
                ...context
            }
        }
    ...
    return (
        <Dialog onClose={handleClose} aria-labelledby="simple-dialog-title" open={open} fullWidth={true}>
            ...
            <DialogContent>
                <Typography style={{fontWeight: "bold"}}>Employement Contract</Typography>
                <Select 
                ...
                >
                    {
                        contracts.map((c, i) => (
                            <MenuItem key={i} value={c.id}>{c.fullName}</MenuItem>
                        ))
                    }
                </Select>
